...
const Navbar = () => {
  ...
  const { signin, signup, setSignin, setSignup } = useContext(AuthModalContext);
  ...
  return (
    <>
      <nav className="w-full flex flex-row items-center justify-between px-5 py-4 relative">
        ...
        {storedUser ? (
            <div className=" md:hidden text-[#605DEC] text-base">
              <p className="font-medium md:font-bold sm:text-base">Hello, {storedUser}</p>
            </div>
        ) : (
        <div className="">
          <button className="block md:hidden bg-[#605DEC] py-2 px-4 md:py-3 md:px-5 rounded-[5px] border-2 border-[#605DEC] text-base text-[#FAFAFA] hover:text-[#605DEC] hover:bg-white hover:border-2 hover:border-[#605DEC] transition-all duration-200" onClick={() => { setSignin(!signin); setSignup(false); }}>Sign in</button>
          { signin && (
          <Signin
            signin={signin}
            setSignin={(show) => {
              setSignin(show);
              if (show) setSignup(false);
            }}
            signup={signup}
            setSignup={setSignup}
          />
        ...
        <div className="hidden md:flex items-center space-x-8">
          ...
          {storedUser ? (
            <div className="text-[#605DEC] text-base">
              <p className="font-medium md:font-bold sm:text-base">Hello, {storedUser}</p>
            </div>
          ) : (
          <div className="">
            <button className="bg-[#605DEC] py-2 px-4 md:py-3 md:px-5 rounded-[5px] border-2 border-[#605DEC] text-base text-[#FAFAFA] hover:text-[#605DEC] hover:bg-white hover:border-2 hover:border-[#605DEC] transition-all duration-200" onClick={() => { setSignin(!signin); setSignup(false); }}>Sign in</button>
            { signin && (
              <Signin
                signin={signin}
                setSignin={(show) => {
